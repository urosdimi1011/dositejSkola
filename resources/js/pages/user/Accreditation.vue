<template>
    <my-layout>
        ...
        <div class="documents-block mx-auto max-w-screen-xl grid grid-cols-1 sm:grid-cols-2">
            <div class="document my-5" v-for="document in documents" :key="document.id">
                <h4 class="!text-sm sm:text-md"><a class="hover:text-red-800 transition flex items-center gap-2" :href="'/assets/files/'+document.href"><PhFile :size="22" /> {{document.name}} </a></h4>
            </div>
        </div>
    </my-layout>
</template>
